# In this file you can perform SARIMAX, RandomForest, XGBoost and GBM models to predict the evaporation using many variables.
...
# Importing all covariates
liste_var = c("daily_Sum_Temp2M.csv", "daily_Sum_Radiation.csv", "daily_Sum_SurfacePressure.csv", "daily_Sum_SoilTemp4.csv", "daily_Sum_SoilTemp1.csv", "daily_Sum_LakeBottomTemp.csv", "daily_Sum_vWind.csv", "daily_Sum_uWind.csv", "daily_Sum_TotPrecipitation.csv")
...
# Grouping everything by month
full_data <- full_data %>%
  group_by(date = lubridate::floor_date(date, 'month')) %>%
  summarize(across(c("evaporation_from_open_water_surfaces_excluding_oceans_sum","Temp2M", "Radiation", "SurfacePressure", "SoilTemp4", "SoilTemp1", "LakeBottomTemp", "vWind", "uWind", "TotPrecipitation"), sum, na.rm = TRUE))

# Adding year and month
full_data$year <- year(full_data$date)
full_data$month <- month(full_data$date)

# Creating lag variables for every variables
for (col in c("evaporation_from_open_water_surfaces_excluding_oceans_sum", "Radiation", "SurfacePressure", "SoilTemp4", "SoilTemp1", "LakeBottomTemp", "vWind", "uWind", "TotPrecipitation")) {
...
# Dropping not needed columns anymore
columns_to_drop <- c("Temp2M", "Radiation", "SurfacePressure", "SoilTemp4", "SoilTemp1", "LakeBottomTemp", "vWind", "uWind", "TotPrecipitation")
  ...
  for (p in 0:maxord[1]) for(d in 0:maxord[2]) for(q in 0:maxord[3])
    for (P in 0:maxord[4]) for(D in 0:maxord[5]) for(Q in 0:maxord[6])
    {
      ...
      fit <- Arima(y = x.ts, 
                   ...
                   xreg = exogenous_variable) # Adding the covariates
...
# Creating the covariates matrix of train data
...
myModel <- Arima(y, order = c(0, 0, 1), seasonal = c(2, 0, 1), lambda = NULL, include.constant = TRUE, xreg = exogenous_variable)
...
# Creating exogenous matrix for test sample
...
d.forecast <- forecast(myModel, level = c(95), h = 1, xreg = exogenous_variable_test)
autoplot(d.forecast)
...
### Pre process for machine learning models
...
randomforestModel <- randomForest(
  formula,
  data = train_data,
  ntree = 2000,            # You can adjust the number of trees
  mtry = 12
)
...
y_pred_gbm <- predict(gbm_model, newdata = test_data)
